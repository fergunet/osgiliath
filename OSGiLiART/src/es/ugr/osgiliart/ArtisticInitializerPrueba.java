/*
 * ArtisticInitializerPrueba.java
 * 
...
import es.ugr.osgiliart.primitives.basic.generators.QuadGenerator;
...
public class ArtisticInitializerPrueba extends OsgiliathService implements Initializer {
	...
	protected ArtisticIndividual newIndividual () {		
		...
		QuadGenerator quadGenerator = new QuadGenerator(pointGenerator, colorGenerator);
		

		int N = (Integer) this.getAlgorithmParameters().getParameter(ArtisticParameters.GENOME_SIZE);
		for ( int i = 0; i < N; ++i ) {
			primitives.add( quadGenerator.generate() );
		}
