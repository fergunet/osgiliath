...
import es.ugr.osgiliath.problem.ndimfunctions.evolutionary.implementations.BLXaListCrossover;

public class benchmark {

	// Fixed (class) parameters
	static final public int NUM_TEST_FUNC = 25;
	static final public String DEFAULT_FILE_BIAS = "/cec05/supportData/fbias_data.txt";
	...
	static final public double[] max_values = {
			100,	//"F01_shifted_sphere",
			100,	//"F02_shifted_schwefel",
			100,	//"F03_shifted_rotated_high_cond_elliptic",
			100,	//"F04_shifted_schwefel_noise",
			100,	//"F05_schwefel_global_opt_bound",
			100,	//"F06_shifted_rosenbrock",
			600,	//"F07_shifted_rotated_griewank", init only
			 32,	//"F08_shifted_rotated_ackley_global_opt_bound",
			  5,	//"F09_shifted_rastrigin",
			  5,	//"F10_shifted_rotated_rastrigin",
			0.5,	//"F11_shifted_rotated_weierstrass",
		Math.PI,	//"F12_schwefel",
			  1,	//"F13_shifted_expanded_griewank_rosenbrock",
			100,	//"F14_shifted_rotated_expanded_scaffer",
			  5,	//"F15_hybrid_composition_1",
			  5,	//"F16_rotated_hybrid_composition_1",
			  5,	//"F17_rotated_hybrid_composition_1_noise",
			  5,	//"F18_rotated_hybrid_composition_2",
			  5,	//"F19_rotated_hybrid_composition_2_narrow_basin_global_opt",
			  5,	//"F20_rotated_hybrid_composition_2_global_opt_bound",
			  5,	//"F21_rotated_hybrid_composition_3",
			  5,	//"F22_rotated_hybrid_composition_3_high_cond_num_matrix",
			  5,	//"F23_noncontinuous_rotated_hybrid_composition_3",
			  5,	//"F24_rotated_hybrid_composition_4",
			  5,	//"F25_rotated_hybrid_composition_4_bound" init only
	};

		static final public double[] min_values = {
			-100,	//"F01_shifted_sphere",
			-100,	//"F02_shifted_schwefel",
			-100,	//"F03_shifted_rotated_high_cond_elliptic",
			-100,	//"F04_shifted_schwefel_noise",
			-100,	//"F05_schwefel_global_opt_bound",
			-100,	//"F06_shifted_rosenbrock",
			   0,	//"F07_shifted_rotated_griewank",
			 -32,	//"F08_shifted_rotated_ackley_global_opt_bound",
			  -5,	//"F09_shifted_rastrigin",
			  -5,	//"F10_shifted_rotated_rastrigin",
			-0.5,	//"F11_shifted_rotated_weierstrass",
		-Math.PI,	//"F12_schwefel",
			  -3,	//"F13_shifted_expanded_griewank_rosenbrock",
			-100,	//"F14_shifted_rotated_expanded_scaffer",
			  -5,	//"F15_hybrid_composition_1",
			  -5,	//"F16_rotated_hybrid_composition_1",
			  -5,	//"F17_rotated_hybrid_composition_1_noise",
			  ...
			  -5,	//"F23_noncontinuous_rotated_hybrid_composition_3",
			  -5,	//"F24_rotated_hybrid_composition_4",
			   2,	//"F25_rotated_hybrid_composition_4_bound"
	};
	...
	static final public Random random = new Random(123456789);
	...
	public test_func testFunctionFactory(int func_num, int dimension) {
		...
		try {
			...
			Class c = this.getClass().getClassLoader().loadClass("cec05."+test_func_class_names[func_num-1]);
			if(c!=null)
				System.out.println("C vale "+c.getCanonicalName());
			else
				System.out.println("C = NULL");
			returnFunc = (test_func )
				//loader.loadClass("cec05."+test_func_class_names[func_num-1])
					c.getConstructor(test_func_arg_types)
					.newInstance(
						new Object[] {
	...
	public void runTest(int func_num) {
		if (func_num == 0) {
			...
			String file_test = "src/cec05/testData/test_data_func" + func_num + ".txt";
	...
	public double get_max_value(int num_func){
		return max_values[num_func-1];
	}

	public double get_min_value(int num_func){
		return min_values[num_func-1];
	}
	//
	// Basic functions
	//
	...
	static public void loadRowVectorFromFile(String file, int columns, double[] row) {
		try {
			BLXaListCrossover d = new BLXaListCrossover();
			System.out.println("FILE "+file);
			if(file.startsWith("src")){
				file = file.replace("src/", "/");
				System.out.println("ARREGLADO A "+file);
			}
			InputStream in = d.getClass().getResourceAsStream(file); //Tiene que coincidir con fileBIAS
			System.out.println("IN TIENE "+in);
			//BufferedReader brSrc = new BufferedReader(new FileReader(file));
			BufferedReader brSrc = new BufferedReader(new InputStreamReader(in));
